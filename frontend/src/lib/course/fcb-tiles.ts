 ...
 * there is no real F/C/B geometry to show — that is EITHER:
 *   - the resolved source is already `card` (tee-anchor's card-only fallback), OR
 *   - `fcb` is null: no live GPS fix AND no from-tee geometry (unmapped /
 *     anchor-only round, a hole absent from the coords, or the pre-load window).
 *
 * Otherwise the three real distances from `fcb` (live GPS "you", or from-tee).
 * The three working paths are byte-identical to the pre-extraction behavior:
 * on the GPS and from-tee paths `fcb` is always non-null, and on the card path
 * `fcbSource === 'card'` — so `fcb!` is always safe in the else branch.
 */
export function buildFcbTiles(opts: {
  ...
  const cardOnly = fcbSource === 'card' || fcb == null;
  if (cardOnly) {
