    ...
    <PaperShell>
      ...
      <Section kicker="Where" title="Courses" aside={<Kicker>{selectedCount} of {courses.length}</Kicker>}>
        ...
        <div>
          ...
          <div style={{ fontFamily: T.mono, fontSize: 8.5, letterSpacing: 1.3, color: T.pencilSoft, textTransform: "uppercase" as const, fontWeight: 500, marginTop: 14, marginBottom: 4 }}>
            Open to
          </div>
          {courses.filter((c) => !c.favorite).map((c, i) => (
            <CourseRow key={c.id} c={c} accent={accent} onToggle={() => toggleCourse(c.id)} first={i === 0} />
...
function Searching({ accent, windows, courses, maxMiles, group, onBack, onFound }: SearchingProps) {
  const [elapsedSec, setElapsedSec] = useState(0);
  const [log, setLog] = useState<LogLine[]>([]);
  const [error, setError] = useState<string | null>(null);
  const done = useRef(false);

  const targetDate       = nextSaturday();
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const queries: TeeTimeQuery[] = selectedWindows.map((w) => ({
        date: targetDate,
      ...
      if (queries.length === 0) {
        queries.push({ date: targetDate, timeWindowStart: "06:00", timeWindowEnd: "12:00", partySize, maxDistanceMiles: maxMiles });
      }
...
function Confirmed({ accent, slot, bookingResult, group, onBack, onStart }: ConfirmedProps) {
  if (!slot) {
    return (
      <PaperShell>
  ...
  const teeTime   = formatTime12h(slot.time);
  ...
  return (
    <PaperShell>
      <div style={{ padding: "max(14px, env(safe-area-inset-top)) 22px 12px", position: "relative" }}>
        ...
        >
          ...
          <div style={{ fontFamily: T.serif, fontStyle: "italic", fontSize: 22, letterSpacing: -0.4, lineHeight: 1, marginTop: 2 }}>
            {bookingResult?.status === "confirmed" ? "Booked" : bookingResult?.status === "pending" ? "Pending" : "Found"}
          </div>
      ...
      <div style={{ padding: "0 22px 14px" }}>
        <Transcript accent={accent} lines={[{
          who: "looper",
          text: `Found one. ${teeTime} at ${slot.courseName}${slot.cartIncluded ? ", cart included" : ", walking"}.${isMock ? " (Demo data.)" : ""}`,
        }]} />
      </div>

      {/* Booking link for mock / affiliate (not_supported) */}
      {slot.bookingUrl && (
        <div style={{ padding: "0 22px 14px" }}>
          <a
  ...
  return (
    <div style={{ position: "relative", width: 200, height: 200, margin: "0 auto" }}>
      <svg width="200" height="200" viewBox="0 0 200 200">
        ...
        <g><circle cx="70" cy="72" r="3" fill={accent} /><text x="78" y="75" fontFamily={T.mono} fontSize="7" fill={T.pencil} letterSpacing="1">PRESIDIO</text></g>
        <g><circle cx="132" cy="60" r="3" fill={T.ink} /><text x="140" y="63" fontFamily={T.mono} fontSize="7" fill={T.pencil} letterSpacing="1">HARDING</text></g>
        <g><circle cx="58" cy="130" r="3" fill={T.ink} /><text x="8" y="133" fontFamily={T.mono} fontSize="7" fill={T.pencil} letterSpacing="1">LINCOLN</text></g>
