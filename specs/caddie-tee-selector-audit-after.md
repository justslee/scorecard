# Tee-selector audit — AFTER (all-courses, read-only, prod DB)

Same script, same read-only prod DB run, immediately following the BEFORE
audit (`caddie-tee-selector-audit-before.md`) — the ONLY difference is
`app/caddie/aim_point.py`'s bend-cap `corner_trees` filter, which now also
requires `h.carry_yards <= bend.distance_yards + CORNER_TREE_FORWARD_YDS`
(40y) — closing the missing-upper-bound gap the BEFORE audit convicted.

To measure this WITHOUT deploying to prod or touching the live app
directory (`/home/ubuntu/scorecard`), the fixed code was run from an
isolated `/tmp` copy on the same box (`app`/`scripts` copied read-only from
the deployed tree, the ONE file overlaid with the fix, then run against the
SAME read-only `DATABASE_URL`) — the running `scorecard-api` service and its
on-disk code were never modified. The overlay was verified byte-identical
to the fix committed in this bundle before running.

168 par-4/5 holes audited (same set, byte-identical to BEFORE). **20 → 14
flagged.**

## Diff vs BEFORE (verified programmatically, not by eye)

- **0 rows added or removed** (same 168 holes, same course set).
- **9 rows changed** — every single one moved from a CAPPED pick to
  `driver` (uncapped) on BOTH bags where applicable. **Zero rows moved the
  other direction** (no previously-uncapped hole became newly capped) —
  the fix only REMOVES fabricated caps, never invents one.
- **6 FLAG rows cleared** (all 6 BOGUS-judged rows from the BEFORE table):
  Pebble Beach 3, Pine Valley 1, Pine Valley 9, Pine Valley 16, Kiawah
  Island 3, Shinnecock Hills 15.
- **3 more rows changed pick (3wood → driver) without being FLAG-eligible
  before or after** — the milder bogus cases: Augusta National 13, Bethpage
  Black 16, Cypress Point 1. Confirms the bug's effect was continuous (not
  only in the FLAG-severity range) and the fix removes it uniformly.
- **Every LEGIT row is byte-identical to BEFORE** — same pick, same
  mechanism, same FLAG status: Pebble Beach 1/16, Bethpage Black 1/2/6/7,
  Kiawah Island 2, Augusta National 2/10, Pinehurst No. 2 14, Muirfield
  Village 14, St Andrews 1. `test_corridor_bend_cap.py` (all 6, incl. the
  reckless-driver-through-trees guard `test_corridor_caps_club_short_of_the
  _corner`), `test_corridor_width_selection.py` (01-08), and
  `test_tee_club_expected_strokes.py`'s protected list (`test_05`,
  `test_05b`, `test_06`, `test_07`, `test_08`, `test_13_red6_5iron_via_bend
  _cap_unchanged`) all pass unchanged (see gate output in the PR/session
  report) — this table is the systemic, whole-fleet proof the same
  guardrail tests only sample.
- **14 remaining flagged rows are all LEGIT or false positives of this
  audit's own crude FLAG heuristic** (reachable/approach-path picks, not
  the bend-cap/corridor mechanism) — see the BEFORE file's verdict summary,
...
| Pebble Beach Golf Links | 3 | 4 | 381(derived) | driver | driver | none | left@265/48y | 350y(+85 vs corner)/moderate/right; 390y(+125 vs corner)/moderate/right; 405y(+140 vs corner)/moderate/right | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |
...
| Bethpage Black | 16 | 4 | 481(derived) | driver | driver | none | left@295/23y | 350y(+55 vs corner)/moderate/left; 370y(+75 vs corner)/moderate/left | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |

| Bethpage Black | 18 | 4 | 508(derived) | driver | driver | none | left@395/41y | none | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |
| Pine Valley | 1 | 4 | 349(derived) | driver | driver | none | right@240/103y | 300y(+60 vs corner)/moderate/right; 330y(+90 vs corner)/moderate/right; 355y(+115 vs corner)/moderate/right; 375y(+135 vs corner)/moderate/right; 410y(+170 vs corner)/moderate/right; 435y(+195 vs corner)/moderate/right; 450y(+210 vs corner)/moderate/center | L:0obs/0feat/q=n R:1obs/1feat/q=n |  |
...
| Pine Valley | 9 | 4 | 554(derived) | driver | driver | none | left@175/43y | 455y(+280 vs corner)/moderate/left; 470y(+295 vs corner)/moderate/left; 495y(+320 vs corner)/moderate/left; 535y(+360 vs corner)/moderate/left; 570y(+395 vs corner)/moderate/left; 580y(+405 vs corner)/moderate/left | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |
...
| Pine Valley | 16 | 4 | 473(derived) | driver | driver | none | right@225/63y | 285y(+60 vs corner)/moderate/right; 380y(+155 vs corner)/moderate/right | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |
...
| Cypress Point Club | 1 | 4 | 414(derived) | driver | driver | none | right@295/18y | 360y(+65 vs corner)/moderate/right; 400y(+105 vs corner)/moderate/right; 420y(+125 vs corner)/moderate/right | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |
...
| Kiawah Island (Ocean Course) | 3 | 4 | 357(derived) | driver | driver | none | left@245/49y | 310y(+65 vs corner)/moderate/left; 330y(+85 vs corner)/moderate/left | L:1obs/1feat/q=n R:0obs/0feat/q=n |  |
...
| Shinnecock Hills | 15 | 4 | 395(derived) | driver | driver | none | right@240/44y | 360y(+120 vs corner)/moderate/right; 425y(+185 vs corner)/moderate/right | L:0obs/0feat/q=n R:0obs/0feat/q=n |  |
...
| Augusta National | 13 | 5 | 451(derived) | driver | driver | none | left@285/130y | 330y(+45 vs corner)/moderate/left; 400y(+115 vs corner)/moderate/left; 450y(+165 vs corner)/moderate/left; 515y(+230 vs corner)/moderate/left; 585y(+300 vs corner)/moderate/left; 660y(+375 vs corner)/moderate/left; 700y(+415 vs corner)/moderate/left | L:1obs/1feat/q=n R:0obs/0feat/q=n |  |
...
**Totals:** 168 par-4/5 holes audited, 14 flagged.
