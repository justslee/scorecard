...
2. **Availability-call jobs:** stamp `user_id` into `_availability_jobs[job_id]` at creation; `GET /availability-call/{job_id}` returns 404 unless the stamp matches the caller (uuid4 unguessability is not authorization). Apply the identical stamp-and-match pattern to scorecard-OCR job ids and any course-search async job.
...
**Cycle count: ~3–4.** (1: backend flip + gap closures + isolation suite; 1: migrations design/verify + webhook/revocation; 1–1.5: client identity + namespacing + Keychain; 0.5: hardening + review remediation.)
...
- P0 design threat-model: reviewed by a `/security-review`-minded reviewer pass (findings folded into §3.4/§3.6).
