...
async def _import_tournaments(
    ...
    for rec in records:
        ...
        for g in games:
            ...
            raw_game_player_ids: list[str] = g.get("playerIds") or []
            remapped_game_player_ids = [
                player_id_map.get(pid, pid) for pid in raw_game_player_ids
            ]
            gm_stmt = (
                pg_insert(GameORM)
                .values(
...
async def _import_rounds(
    ...
    for rec in records:
        ...
        if legacy_tournament_id:
            candidate = tournament_id_map.get(legacy_tournament_id, legacy_tournament_id)
            # Only set tournament_id if it resolves to a valid UUID (FK requirement).
            try:
                uuid.UUID(candidate)
                remapped_tournament_id = candidate
            except ValueError:
                log.warning(
                    "    round %r: tournamentId %r could not be resolved to a UUID — "
                    "setting tournament_id=NULL",
...
async def _patch_tournament_round_ids(
    ...
    Only runs if there are tournament records with non-empty roundIds lists.
    Skips rounds not found in round_id_map (e.g. rounds imported separately or
    referenced but not present in rounds.json).
    """
    ...
    for rec in tournament_records:
        ...
        if not raw_round_ids:
            continue
        remapped_round_ids = [round_id_map.get(rid, rid) for rid in raw_round_ids]
...
async def main() -> int:
    ...
    async with async_session() as db:
        ...
        if tournament_records and round_records:
