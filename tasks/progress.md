...
- AWAITING REVIEWER + QA on 34095c8: reviewer checks allowlist-validation (no arbitrary string→API), precedence owner→env→default, caller INERT (no gating/pipeline/dialog/compliance change), endpoint owner-gated, migration additive, best-effort DB read isn't silently swallowing the owner's pick in prod, tests genuine. QA runs full gates + confirms CI DB-route tests state:SUCCESS on head. SHIP→designer+bundle; BLOCKING→re-dispatch builder.
