...
- AWAITING BUILDER FIX on frontend/src/app/settings/page.tsx CallerVoicePicker: (a) load-failure own honest copy + never render optionless select (mirror 'loading' fallback), (b) field label T.pencil, (c) select padding '10px 12px'. Then designer re-check + re-verify CI strict-green on new head. SHIP→bundle+backlog+DONE; BLOCKING→re-loop.
