...
- AWAITING reviewer on 87c8ee1 (fresh security lens: right seam / guide+hazards both present / no dropping valid guides against empty hazards / both mouths / no crash on absent / RED-genuine). SHIP → update PR checklist + backlog shipped + push. BLOCKING → re-dispatch builder, re-review. Then QA gates strict-green on head, SILENT (no ship/ping).
